...
# Encabezados EXACTOS esperados en la base de datos (respetando mayúsculas y acentos)
TARGET_COL = "Ventas"
FEATURES_COLS = ["PIB", "Empleo", "TipoCambioPct", "Inflación"]
...
def _looks_transposed(df: pd.DataFrame) -> bool:
    """
    Heurística:
    - Si no aparecen variables esperadas en columnas,
    ...
    expected_norm = set(_norm(k) for k in [TARGET_COL] + FEATURES_COLS)
...
def _transpose_if_needed(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Caso típico: primera columna trae nombres de variables y el resto son periodos
    df2 = df.copy()
    first_col = df2.columns[0]
    # Si la primera columna parece etiquetas, usarla como índice y transponer
...
def _multiple_regression(df: pd.DataFrame, y_col: str, x_cols: List[str], x_forecast: Dict[str, float]) -> MultiRegResult:
    ...
    yhat = None
    if all((c in x_forecast and np.isfinite(x_forecast[c])) for c in cols):
...
with st.sidebar:
    ...
    st.caption("El app usa EXACTAMENTE estos encabezados: Ventas, PIB, Empleo, TipoCambioPct, Inflación (respetando mayúsculas y acentos).")
...
needed_cols = [TARGET_COL] + FEATURES_COLS
if NUMERIC_COERCE:
    df = _coerce_numeric(df, needed_cols)

df = _drop_na_rows(df, [TARGET_COL])  # al menos ventas

missing = [c for c in needed_cols if c not in df.columns]
if missing:
    st.warning(
        f"No encontré estas variables en columnas: {missing}. "
...
if mode == "Manual (1 periodo)":
    ...
    fdf = fraw
    if NUMERIC_COERCE:
        fdf = _coerce_numeric(fdf, FEATURES_COLS)
    forecast_df = fdf.copy()

    st.subheader("Pronósticos cargados (X)")
    st.dataframe(forecast_df, use_container_width=True)

    # Tomar la primera fila como escenario principal para resultados “en vivo”
...
corr_rows = []
for x in FEATURES_COLS:
    if x in df.columns and TARGET_COL in df.columns:
        corr = df[[x, TARGET_COL]].corr().iloc[0, 1]
        corr_rows.append({"Variable": x, "Correlacion_con_Ventas": float(corr)})
corr_table = pd.DataFrame(corr_rows)

# Regresiones simples
simples = _simple_regressions(
    df.dropna(subset=[TARGET_COL] + [c for c in FEATURES_COLS if c in df.columns]),
    TARGET_COL,
    FEATURES_COLS,
    x_forecasts,
)
...
with c1:
    st.subheader("Correlaciones con Ventas")
...
if forecast_df is not None:
    cols_present = [c for c in FEATURES_COLS if c in forecast_df.columns and c in df.columns]
    if cols_present and TARGET_COL in df.columns:
        batch_rows = []
        base_df = df.dropna(subset=[TARGET_COL] + cols_present).copy()

        # entrenar modelos una vez
        y = base_df[TARGET_COL].values
        multi_model = LinearRegression().fit(base_df[cols_present].values, y)

        simple_models = {}
        simple_r2 = {}
        for x in cols_present:
            m = LinearRegression().fit(base_df[[x]].values, y)
            ypred = m.predict(base_df[[x]].values)
            simple_models[x] = m
        ...
        for i, row in forecast_df.iterrows():
            xf = {x: row.get(x, np.nan) for x in cols_present}
            if any(pd.isna(xf[x]) for x in cols_present):
                continue

            # simples
            yhat_sim = {x: float(simple_models[x].predict(np.array([[float(xf[x])]]))[0]) for x in cols_present}
            yhat_w = float(sum(weights[x] * yhat_sim[x] for x in cols_present)) if denom > 0 else np.nan

            # múltiple
            Xf = np.array([[float(xf[x]) for x in cols_present]])
            yhat_m = float(multi_model.predict(Xf)[0])

            batch_rows.append({
                "Fila": int(i),
                **{x: float(xf[x]) for x in cols_present},
